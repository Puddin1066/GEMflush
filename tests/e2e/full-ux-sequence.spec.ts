...
test.describe('Full UX Sequence - IDEAL Implementation', () => {
  ...
  test('IDEAL: complete UX sequence with automatic processing and real-time component updates', async ({ 
    ...
    // ========================================================================
    await test.step('Step 1: Create Business (URL Only)', async () => {
    ...
    await test.step('Step 1.5: Handle Location (If Required)', async () => {
    ...
    // ========================================================================
    await test.step('Step 2: Automatic Crawl Verification', async () => {
    ...
    // ========================================================================
    await test.step('Step 3: Automatic Fingerprint Verification', async () => {
    ...
    // ========================================================================
    let finalBusiness: any;
    await test.step('Step 4: Automatic Publish Verification', async () => {
        ...
        finalBusiness = await businessResponse.json().then((r: any) => r.business);
        
        // IDEAL: Business should be published with QID
        expect(finalBusiness.status).toBe('published');
        expect(finalBusiness.wikidataQID).toBeTruthy();
        console.log(`[IDEAL TEST] Publish completed automatically for business ${businessId} with QID: ${finalBusiness.wikidataQID}`);
    ...
    // ========================================================================
    await test.step('Step 5: Final Data Fidelity Verification', async () => {
        console.log('[IDEAL TEST] Step 5: Verifying all components display correct data...');

        // Get final business state from API
        // Note: finalBusiness already fetched in Step 4, but we'll fetch fresh state for deep inspection
        ...
        const businessState = await finalBusinessResponse.json().then((r: any) => r.business);

        // IDEAL: Business should have complete crawl data
        expect(businessState.crawlData).toBeDefined();
        expect(businessState.status).toBeTruthy();
        console.log(`[IDEAL TEST] Final business status: ${businessState.status}`);
        ...
        expect(businessState.wikidataQID).toBeTruthy();
        ...
        expect(entity.qid).toBe(businessState.wikidataQID);
    ...
    // ========================================================================
    await test.step('Step 6: UI Component Verification', async () => {
