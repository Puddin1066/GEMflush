...
describe('Business Lifecycle Integration', () => {
  it('free user creates business → crawls → cannot publish (403)')
  it('free user upgrades → crawls → can publish')
  it('pro user creates business → crawls → fingerprints → publishes')
  it('business status transitions: pending → crawled → published')
...
### 🎭 **Generator Agent** - Create E2E Tests
...
### 🎭 **Healer Agent** - Auto-Repair Failing Tests
...
### 🔴 **Critical (Must Have)**
1. ✅ API endpoint coverage (unit/integration)
2. ✅ Permission checks (free vs pro tier)
3. ✅ Subscription upgrade flow (complete)
4. ✅ Wikidata publishing flow (complete)
5. ❌ Cross-feature integration tests
6. ❌ Data consistency tests

### 🟡 **Important (Should Have)**
1. ✅ Error handling (basic)
2. ❌ Edge cases (race conditions, boundaries)
...
1. ✅ **Immediate**: Run existing test suite to identify current gaps
2. ⏳ **Short-term**: Set up Playwright Test Agents
3. ⏳ **Medium-term**: Generate test plans for missing flows
4. ⏳ **Long-term**: Maintain tests using Healer agent
