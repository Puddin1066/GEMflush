...
export default async function PricingPage() {
  ...
  try {
    // Fetch user and team data (may fail if not authenticated)
    try {
      user = await getUser();
      team = await getTeamForUser();
    } catch (error) {
      // User not authenticated or database error - continue with null values
      console.error('[PricingPage] Error fetching user/team:', error);
    }

    // Fetch Stripe data (may fail if Stripe not configured)
    try {
      ...
      console.error('[PricingPage] Error fetching Stripe data:', error);
      // Continue with empty arrays - page will show "Price Unavailable"
    }
    ...
    return (
      <main className="py-12">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 text-center">
          ...
          {/* Production Error Notice */}
          {!hasPrices && process.env.NODE_ENV === 'production' && (
            <div className="mt-8 max-w-2xl mx-auto">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-left">
                <h3 className="font-semibold text-red-900 mb-2">⚠️ Pricing Temporarily Unavailable</h3>
                <p className="text-sm text-red-800 mb-3">
                  We're experiencing issues loading pricing information. Please try again later or contact support.
                </p>
                <p className="text-sm text-red-700">
                  If this persists, check that STRIPE_SECRET_KEY is configured in your Vercel environment variables.
                </p>
              </div>
            </div>
          )}
    ...
    // Fallback error page
    console.error('[PricingPage] Unexpected error:', error);
    return (
      <main className="py-12">
        <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-gray-600 mb-8">
            We're having trouble loading the pricing page. Please try again later.
          </p>
          <Link href="/">
